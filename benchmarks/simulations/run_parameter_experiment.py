...
def simulate(
    p_mask_node: float, coverage_level: float, random_seed: int
...
def run_experiment(p_mask_node: float, coverage_level: float, num_simulations: int = 100) -> None:
...
def main():
    for p_mask_node in [0.0, 0.2, 0.4, 0.6]:
        for coverage_level in [0.1, 0.2, 0.5, 1.0]:
            run_experiment(p_mask_node=p_mask_node, coverage_level=coverage_level, num_simulations=100)
